...
const { isLinkedToCdpWeb } = require('../tools/cdp-local');

// When @coinbase/cdp-app-attest is symlinked into ../cdp-web, Node follows the
// symlink during Expo autolinking and can pick up expo-modules-core from
// cdp-web/node_modules (often the wrong major). Pre-declaring the pod before
// use_expo_modules! makes autolinking skip its own resolution.
...
function withCdpAppAttest(config) {
  return withDangerousMod(config, [
    ...
    (config) => {
      ...
      // Symlink-only workaround (see EXPO_MODULES_CORE_POD above).
      if (
        isLinkedToCdpWeb('@coinbase/cdp-app-attest') &&
        !contents.includes("pod 'ExpoModulesCore'")
      ) {
        contents = contents.replace(/^  use_expo_modules!$/m, EXPO_MODULES_CORE_POD);
      }

      // Always apply: needed for CdpAppAttest + prebuilt RN regardless of npm/local.
